...
export const ChatProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        // Fetch all users
        const usersResponse = await fetch('https://api.soeparnocorp.workers.dev/users', {
          headers: {
            'X-Session-Id': localStorage.getItem('session') || '',
          }
        });
        ...
        // Fetch online users
        const onlineResponse = await fetch('https://api.soeparnocorp.workers.dev/users/online', {
          headers: {
            'X-Session-Id': localStorage.getItem('session') || '',
          }
        });
  ...
  const loadChannelMessages = async (channelId: string, before?: string) => {
    try {
      ...
      const url = new URL(`https://api.soeparnocorp.workers.dev/channels/${channelId}/messages`);
      if (before) {
